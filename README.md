...
The system includes an automated benchmarking suite to measure cryptographic throughput. Below are the results obtained on an AMD Ryzen 3 3300X @ 3.8GHz (Release Build), demonstrating high-speed processing and efficient bit-level optimizations.

| Algorithm | Throughput (MB/s) | Note |
| :--- | :--- | :--- |
| Simple Substitution | 2404.60 MB/s | Near-instant memory-speed stream processing. |
| MD5 Hashing | 382.43 MB/s | High-speed integrity verification (custom implementation). |
| A5/2 Stream Cipher | 7.57 MB/s | Frame-based GSM encryption (optimized bit-level logic). |
